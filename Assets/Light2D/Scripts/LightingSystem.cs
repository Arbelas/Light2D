...
namespace Light2D {
    ...
    public class LightingSystem : MonoBehaviour {
        ...
        private void RenderAmbientLight() {
            ...
            for (var i = 0; i < _additionalAmbientLightCycles + 1; i++) {
        ...
        private void UpdateCamera() {
            //Calculate new lightPixelSize in case camera size has changed
            ...
            Vector2 gridPos = new Vector2(Mathf.Round(unrotMainPos.x * lightPixelsPerUnityMeter), Mathf.Round(unrotMainPos.y * lightPixelsPerUnityMeter)) / lightPixelsPerUnityMeter;
        ...
        private void ConfigLightCamera(bool extended) {
            if (extended) {
                lightCamera.orthographicSize = _camera.orthographicSize * (_extendedLightTextureSize.y / (float) _smallLightTextureSize.y); // _extendedLightTextureSize.y/(2f*LightPixelsPerUnityMeter);
                lightCamera.fieldOfView = _camera.fieldOfView + lightCameraFovAdd;
